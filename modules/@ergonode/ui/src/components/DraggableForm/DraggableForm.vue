...
<template>
  ...
  >
    ...
    >
      <template #header>
        <h2 v-if="title" class="draggable-form__title" v-text="title" />
      </template>
      <template #body>
...
import { getFixedMousePosition } from "@UI/models/mouse";
...
export default {
  ...
  computed: {
    ...mapState("draggable", ["ghostIndex", "draggedElement"]),
    isPlaceholderItemVisible() {
      return (
        this.hasDropPlaceholder &&
        (!this.localItems.length ||
          (this.localItems.length === 1 &&
            this.draggedElement &&
            this.ghostIndex === -1))
      );
    }
  ...
  methods: {
    ...
    onDragEnter(event) {
      ...
      const { xPos, yPos } = getFixedMousePosition(event);
      const formElement = document
        .elementsFromPoint(xPos, yPos)
        .find(element => element.hasAttribute("index"));

      const index = formElement ? +formElement.getAttribute("index") : 0;

      this.localItems = insertValueAtIndex(
        this.localItems,
        this.draggedElement,
        index
      );
    ...
    onDrop(event) {
      if (
        this.ghostIndex !== -1 &&
        this.localItems.length !== this.items.length
      ) {
        this.$emit("add-item", {
          index: this.ghostIndex,
    ...
    onDragEndItem(index) {
      ...
      const oldItemIndex = this.items.findIndex(
        item => item[optionKey] === newItemId
      );

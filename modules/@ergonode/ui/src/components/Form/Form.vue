...
license details. */
<template>
  <FeedbackProvider :errors="errors" :change-values="changeValues">
    <template #default="{ hasError, hasValueToSave }">
      ...
      >
        <slot name="header">
          <h2 v-if="title" class="form__title" v-text="title" />
        </slot>
        <template v-if="hasError">
          <section class="form__errors" data-cy="form-errors">
        ...
        <slot name="body" :presentation-errors="presentationErrors" />
        <div class="form__footer" v-if="isFooterVisible">
          <slot name="submit">
            <Button
            ...
            >
              <template #append="{ color }">
                <IconSpinner v-if="isSubmitting" :fill-color="color" />
                <IconSync v-else-if="hasValueToSave" :fill-color="color" />
              </template>
            </Button>
          </slot>
          <slot name="proceed">
            ...
            >
              <template v-if="isProceeding" #prepend="{ color }">
...
import { RED } from "@UI/assets/scss/_js-variables/colors.scss";
...
export default {
  ...
  computed: {
    ...
    isFooterVisible() {
      return (
        !!(this.$slots.submit || this.$slots.proceed) ||

...
<template>
  <ResizeObserver @resize="onResize">
    <div class="complex-progress-bar">
      <span class="complex-progress-bar__label" v-text="label" />
...
<script>
import { GRAPHITE_DARK } from "@UI/assets/scss/_js-variables/colors.scss";
...
export default {
  ...
  methods: {
    onResize(entry) {
      ...
      const value = this.value > this.maxValue ? this.maxValue : this.value;
      ...
      requestAnimationFrame(() => {
        if (this.$refs.value && this.$refs.progressDivider) {
          ...
          this.$refs.progressDivider.style.transform = `translateX(${progressWidth -
            1}px)`;
        }
      });
    }

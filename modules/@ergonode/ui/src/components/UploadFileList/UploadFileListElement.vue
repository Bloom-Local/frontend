  ...
  <li :class="classes">
    <div class="horizontal-wrapper">
      <div class="vertical-wrapper">
        <span class="upload-file-list-element__file-name" v-text="file.name" />
        <span
          class="upload-file-list-element__file-size"
          v-text="fileProgress"
        />
      </div>
      <IconButton v-if="isError" :size="smallSize" :theme="secondaryPlainTheme">
        <template #icon>
    ...
    <ProgressBar :color="progressColor" :value="file.progressValue" />
  </li>
</template>

<script>
import { SIZE, THEME } from "@Core/defaults/theme";
import { GREEN, RED } from "@UI/assets/scss/_js-variables/colors.scss";
  ...
  methods: {
    onCancelUploading() {
      this.file.cancelToken.cancel(
        `Uploading ${this.file.name} has been canceled`
      );
    }
  }
};

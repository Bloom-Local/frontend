...
license details. */
<template>
  <div class="cell-resizer" :style="styles">
    <div
      v-if="isResizing"
      class="cell-resizer__focus-imitator"
      :style="focusImitatorStyles"
    />
    <div class="cell-resizer__content" @mousedown="initResizingDrag">
      <div
        v-if="isResizing"
...
<script>
const registerResizeEventListeners = () =>
  import("@UI/models/resize/registerResizeEventListeners");
const unregisterResizeEventListeners = () =>
  import("@UI/models/resize/unregisterResizeEventListeners");

export default {
  name: "GridCellResizer",
  inject: ["getGridTableLayoutReference"],
  props: {
    /**
  ...
  methods: {
    ...
    onResize(event) {
      const { pageY } = event;
      const height =
        pageY + this.getGridTableLayoutReference().scrollTop - this.startY;
      const factor = Math.ceil(height / this.startHeight);
      const fixedHeight = factor * this.startHeight;

      if (
        this.position.row + factor < this.position.minRow ||
      ...
      if (height < 0) {
        this.$refs.border.style.height = `${-1 * fixedHeight +
          this.startHeight}px`;
    ...
    onStopResizing() {
      ...
      if (
        absCurrentHeight !== absStartHeight ||
        absCurrentHeight === absStartHeight
      ) {
...
.cell-resizer {
  ...
  &__focus-imitator {
    position: absolute;
    box-shadow: inset 0 0 0 2px $BLOOM;
  }
}
</style>

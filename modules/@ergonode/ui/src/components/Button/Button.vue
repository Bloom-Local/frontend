...
<template>
  ...
  >
    <!--
        ...
        -->
    <slot name="prepend" :color="foregroundColor" />
    <span v-if="title" :class="titleClasses" v-text="title" />
    <!--
            @slot Append element - icon recommended
            @binding {string} color color of an element
        -->
    <slot name="append" :color="foregroundColor" />
    <slot />
  </button>
</template>
...
import { Z_INDEX_LVL_2 } from "@UI/assets/scss/_js-variables/indexes.scss";
...
export default {
  ...
  props: {
    ...
    size: {
      ...
      validator: value =>
        [SIZE.TINY, SIZE.SMALL, SIZE.REGULAR].indexOf(value) !== -1
    },
    /**
  ...
  computed: {
    ...
    classes() {
      return ["button", `button--${this.theme}`, `button--${this.size}`];
    },
    titleClasses() {

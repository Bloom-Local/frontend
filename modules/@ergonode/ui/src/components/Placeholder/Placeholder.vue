...
license details. */
<template>
  <div :class="classes" :style="placeholderStyles">
    <div class="placeholder__description">
      <PlaceholderTitle v-if="title" :size="size" :title="title" />
      <PlaceholderSubtitle v-if="subtitle" :title="subtitle" />
    </div>
    <slot name="action" />
  </div>
</template>
...
<script>
import { ORIENTATION } from "@Core/defaults/layout";
...
export default {
  ...
  props: {
    ...
    size: {
      ...
      validator: value =>
        [SIZE.SMALL, SIZE.REGULAR, SIZE.LARGE].indexOf(value) !== -1
    },
    /**

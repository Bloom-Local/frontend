    ...
    <template #default="{ uuid }">
      ...
      >
        <div
          ...
          @mouseup="onMouseUp"
        >
          <label class="advanced-filter__label" :for="uuid" v-text="title" />
          <input
            :id="uuid"
            ref="input"
          ...
          <div class="advanced-filter__details">
            <span v-if="!value" class="advanced-filter__placeholder">
              Select...
            </span>
            <template v-else>
              <span class="advanced-filter__value" v-text="value" />
              <span
                v-if="parameters"
        ...
        >
          <slot name="dropdown" :on-apply="onApply" />
        </AdvancedFilterDropdown>
      </div>
    </template>
  </InputUUIDProvider>
...
import { getDraggedColumnPositionState } from "@UI/models/dragAndDrop/helpers";
...
export default {
  ...
  methods: {
    ...
    ...mapActions("list", ["setDisabledScopeElement"]),
    onDragStart(event) {
      addElementCopyToDocumentBody({
        event,
        id: this.filterId,
    ...
    onClickOutside({ event, isClickedOutside }) {
      const isClickedInsideActivator = this.$refs.activator.contains(
        event.target
      );

      this.isClickedOutside = isClickedOutside && !isClickedInsideActivator;

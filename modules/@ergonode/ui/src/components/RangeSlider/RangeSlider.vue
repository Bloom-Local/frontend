...
<template>
  <div class="slider">
    ...
    <div
      ...
      @mouseleave="onMouseLeave"
    />
    <div ref="tracker" class="slider__tracker" />
    <div :style="progressStyle" class="slider__progress" />
  </div>
</template>
...
export default {
  ...
  computed: {
    progressStyle() {
      ...
      return {
        transform: `translate(${this.getTransformValue(
          from
        )}px, 0) scale(${this.getTransformValue(distance)}, 1)`
      };
    },
  ...
  methods: {
    onInitResize(event) {
      this.isDraggingUpperSphere = event.target.classList.contains(
        "slider__upper-sphere"
      );

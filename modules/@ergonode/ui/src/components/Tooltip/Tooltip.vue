...
<script>
import { POSITION } from "@Core/defaults/theme";
import { GRAPHITE } from "@UI/assets/scss/_js-variables/colors.scss";
import { getFixedMousePosition, isMouseOutsideElement } from "@UI/models/mouse";
  ...
  methods: {
    ...
    createTooltip() {
      ...
      switch (this.position) {
        case POSITION.TOP:
          positionStyle = `
                    transform: translateY(calc(-100% - ${this.$el.offsetHeight -
                      padding * paddingMultiplier}px));
    ...
    updateFluidTooltipPosition(xPos, yPos) {
      this.tooltip.style.transform = `translate(${xPos -
        this.tooltip.offsetWidth / 2}px, ${yPos -
        8 -
        this.tooltip.offsetHeight}px)`;
    }
  }
};
</script>

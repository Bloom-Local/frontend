        ...
        <template #activator="{ inputReference }">
          <InputController>
            ...
            <!--
                        ...
                        -->
            <slot name="append" />
            <ErrorHint v-if="isError" :hint="errorMessages" />
...
export default {
  ...
  computed: {
    presentingValue() {
      ...
      if (this.optionValue || this.optionKey) {
        return value
          .map(item => item[this.optionValue] || `#${item[this.optionKey]}`)
          .join(", ");
      }
    ...
    height() {
      ...
      return this.size === SIZE.SMALL ? "32px" : "40px";
    },
    styleComponent() {
    ...
    classes() {
      return ["select", `select--${this.alignment}`];
    },
    dropDownState() {
    ...
    placeholderValue() {
      if (!this.hasAnyValueSelected || (this.label && !this.isFocused))
        return null;
    ...
    value: {
      ...
      handler() {
        ...
        if (isArray && this.value.length) {
          ...
          this.hasAnyValueSelected = Boolean(
  ...
  methods: {
    ...
    onClickOutside({ event, isClickedOutside }) {
      const isClickedInsideActivator = this.$refs.activator.$el.contains(
        event.target
      );

/* * Copyright © Ergonode Sp. z o.o. All rights reserved. * See LICENSE for
license details. */
<template>
        ...
        <div class="horizontal-container">
          ...
          <div class="dashboard-header">
            <div class="dashboard-header__description">
              <h1 class="dashboard-header__title" v-text="title" />
              <h2 class="dashboard-header__subtitle" v-text="subtitle" />
            </div>
          </div>
        </div>
          ...
          >
            <template #action>
              <Button
                :title="
                  $t(
                    '@Dashboard.dashboard.components.EmptyDashboardPage.attributeTitle'
                  )
                "
                :size="smallSize"
          ...
          >
            <template #action>
              <Button
                :title="
                  $t(
                    '@Dashboard.dashboard.components.EmptyDashboardPage.templateTitle'
                  )
                "
                :size="smallSize"
          ...
          >
            <template #action>
              <Button
                :title="
                  $t(
                    '@Dashboard.dashboard.components.EmptyDashboardPage.productTitle'
                  )
                "
                :size="smallSize"
...
import { GREY } from "@UI/assets/scss/_js-variables/colors.scss";
...
export default {
  ...
  components: {
    ...
    DoughnutProductsChart,
    CreateAttributeModalForm: () =>
      import("@Attributes/components/Modals/CreateAttributeModalForm"),
    CreateProductModalForm: () =>
      import("@Products/components/Modals/CreateProductModalForm"),
    CreateProductTemplateModalForm: () =>
      import("@Templates/components/Modals/CreateProductTemplateModalForm")
  },
  data() {
    return {
  ...
  computed: {
    ...
    isAllowedToCreateAttribute() {
      return this.$hasAccess([ATTRIBUTE_PRIVILEGES.ATTRIBUTE.create]);
    },
    isAllowedToCreateProduct() {
      return this.$hasAccess([PRODUCT_PRIVILEGES.PRODUCT.create]);
    },
    isAllowedToReadProduct() {
      return this.$hasAccess([PRODUCT_PRIVILEGES.PRODUCT.read]);
    },
    isAllowedToCreateTemplate() {
      return this.$hasAccess([TEMPLATE_PRIVILEGES.TEMPLATE_DESIGNER.create]);
    },
    smallSize() {
      return SIZE.SMALL;
    },
    title() {
      return this.$t(
        "@Dashboard.dashboard.components.EmptyDashboardPage.title"
      );
    },
    subtitle() {
      return this.$t(
        "@Dashboard.dashboard.components.EmptyDashboardPage.subtitle"
      );
    },
    firstStep() {
      return {
        title: "1.",
        subtitle: this.$t(
          "@Dashboard.dashboard.components.EmptyDashboardPage.first.subtitle"
        ),
        description: this.$t(
          "@Dashboard.dashboard.components.EmptyDashboardPage.first.description"
        )
      };
    },
    secondStep() {
      return {
        title: "2.",
        subtitle: this.$t(
          "@Dashboard.dashboard.components.EmptyDashboardPage.second.subtitle"
        ),
        description: this.$t(
          "@Dashboard.dashboard.components.EmptyDashboardPage.second.description"
        )
      };
    },
    thirdStep() {
      return {
        title: "3.",
        subtitle: this.$t(
          "@Dashboard.dashboard.components.EmptyDashboardPage.third.subtitle"
        ),
        description: this.$t(
          "@Dashboard.dashboard.components.EmptyDashboardPage.third.description"
        )
      };
    }
  },
...
.dashboard-stats {
  ...
  height: 358px;
  margin: 40px 0;
  background: url("~@Dashboard/assets/images/dashboard_man.svg") no-repeat left
    bottom;

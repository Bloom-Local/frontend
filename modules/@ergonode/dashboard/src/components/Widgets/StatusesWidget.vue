...
<template>
  ...
  >
    ...
    <template #body>
      ...
      <ul class="list-status" v-else>
        <template v-for="(status, index) in statuses">
          <li class="list-status-element" :key="status.id">
...
import { getStatusesCount } from "@Dashboard/services";
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("core", ["availableLanguages"]),
    ...mapGetters("core", ["defaultLanguageCode"]),
    tinySize() {
      return SIZE.TINY;
    },
    languageOptions() {
      return this.availableLanguages.map(({ name }) => name);
    }
  },
  created() {
    this.workflowLanguage = this.availableLanguages.find(
      ({ code }) => code === this.defaultLanguageCode
    );
  },
  methods: {
    async getStatusesCount() {
      try {
        ...
        this.statuses = statusesCount.map(status => {
          const { status_id, code, value, label, color } = status;
          ...
          return {
            ...
            value:
              value === 1
                ? this.$t(
                    "@Dashboard.dashboard.components.StatusesWidget.productsCountSingularLabel",
                    {
                      info: value
                    }
                  )
                : this.$t(
                    "@Dashboard.dashboard.components.StatusesWidget.productsCountPluralLabel",
                    {
                      info: value
                    }
                  )
        ...
        this.$addAlert({
          ...
          message: this.$t(
            "@Dashboard.dashboard.components.StatusesWidget.getRequest"
          )
        });

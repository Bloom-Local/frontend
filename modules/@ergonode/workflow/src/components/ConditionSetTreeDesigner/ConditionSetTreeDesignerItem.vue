...
<template>
  ...
  >
    <template #body>
      ...
      <div v-else class="condition-set-tree-designer-item__body">
...
import ConditionSetTreeDesignerItemDescription from "@Workflow/components/ConditionSetTreeDesigner/ConditionSetTreeDesignerItemDescription";
import ConditionSetTreeDesignerItemParameter from "@Workflow/components/ConditionSetTreeDesigner/ConditionSetTreeDesignerItemParameter";
import ConditionSetTreeDesignerItemParameters from "@Workflow/components/ConditionSetTreeDesigner/ConditionSetTreeDesignerItemParameters";
import { hasOptions } from "@Workflow/models/conditionDesigner/conditionTypes";
...
export default {
  ...
  computed: {
    ...mapState("workflowConditions", ["conditionsValues"]),
    menuItems() {
      return ["Remove"];
    },
    conditionPhrase() {
  ...
  methods: {
    ...
    replacePlaceholderOnPhrase(placeholders) {
      ...
      const findKeyWhenSelect = clearedKey =>
        parameters.findIndex(
          parameter =>
            parameter.name === clearedKey && hasOptions(parameter.type)
        );

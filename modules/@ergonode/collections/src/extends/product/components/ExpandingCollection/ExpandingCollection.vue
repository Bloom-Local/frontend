...
<template>
  <div class="expanding-collection">
    <div class="expanding-collection__header">
      <div class="prepend-header">
        <slot name="prependHeader">
          <span class="prepend-header__title" v-text="collection.title" />
          <span class="prepend-header__subtitle" v-text="collection.subtitle" />
          <span
    ...
    <div class="expanding-collection__preloader" v-if="isPrefetchingData">
      <Preloader />
    </div>
    <div
      :style="gridTemplate"
    ...
    >
      <template v-for="(item, itemIndex) in collection.items">
        <slot name="item" :item="item" :item-index="itemIndex" />
      </template>
    </div>
  </div>
</template>
...
export default {
    ...
    gridTemplate() {
      ...
      return {
        gridTemplateRows: `repeat(${Math.ceil(
          this.collection.items.length / columnsNumber
        )}, ${width})`
      };
    }
  },
  methods: {
    onExpand(isExpanded) {
      ...
      if (
        this.isExpanded &&
        this.collection.itemsCount !== this.collection.items.length
      ) {
        this.$emit("fetch", {

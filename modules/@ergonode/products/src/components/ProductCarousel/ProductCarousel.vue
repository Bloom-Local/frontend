...
import { ROUTE_NAME } from "@Products/config/routes";
...
export default {
  ...
  watch: {
    fetchingDataIndex: {
      ...
      handler() {
        ...
        if (
          typeof this.products[productId] === "undefined" &&
  ...
  methods: {
    ...
    onFetchDataSuccess({ rows, fetchingIndex }) {
      if (rows.length) {
        const [product] = rows;

      ...
      <div class="upload-file-tab">
        ...
        >
          <input
            ...
            @change="onUpload"
          />
          <label class="upload-file-tab__label" :for="uuid">
            <IconUploadCloudFile
            ...
            <span
              ...
              v-text="
                $t('@Media.media.components.UploadFileTab.informationLabel')
              "
            />
            <template v-if="!isDraggedFileOver">
              <span
                ...
                v-text="
                  $t('@Media.media.components.UploadFileTab.logicOperator')
                "
              />
              <Button
                :size="smallSize"
        ...
        <div class="upload-file-tab__uploading-panel" v-show="files.length">
...
import { getMappedScopedErrors } from "@Core/models/mappers/errorsMapper";
import { isObject } from "@Core/models/objectWrapper";
import { formatBytes } from "@Core/models/stringWrapper";
import { GREEN, WHITE } from "@UI/assets/scss/_js-variables/colors.scss";
      ...
      Object.keys(files).forEach((key, index) => {
        ...
        const config = {
          ...
          onUploadProgress: ({ loaded, total }) => {
            this.files[fixedIndex].progressValue = Math.floor(
              (loaded * 100) / total
            );
        ...
        this.$axios
          .$post("multimedia/upload", formData, config)
          .then(({ id }) => {
            this.$addAlert({
              type: ALERT_TYPE.SUCCESS,
              message: this.$t(
                "@Media.media.components.UploadFileTab.successMessage"
              )
            });
